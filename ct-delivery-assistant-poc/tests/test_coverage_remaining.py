...
def test_settings_validators_raise_on_invalid():
    ...
    with pytest.raises(ValueError):
        Settings._validate_cookie_secure(False, info=_types.SimpleNamespace(data={"cookie_samesite": "none"}))



def test_get_session_handles_invalid_json_and_refresh_ttl(monkeypatch):
    # invalid json -> returns None
    monkeypatch.setattr(core_redis, "redis_client", types.SimpleNamespace(get=lambda k: "not-json", expire=lambda *a, **k: None))
    ...
    monkeypatch.setattr(core_redis, "redis_client", types.SimpleNamespace(get=fake_get, expire=fake_expire))
...
def test_jira_helpers_ensure_sid_and_client_from_session(monkeypatch):
    ...
    session = {"tokens_by_cloud": {}, "cloud_ids": ["c1"], "active_cloud_id": "c1", "access_token": "tok"}
